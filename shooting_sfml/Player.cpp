...
void Player::UiInit()
{
	...
	m_sprite.setColor(Color(10,10,10,200));
...
void Player::Fight(const float& _dt)
{
	...
	{
		m_vecbullet.push_back(Bullet(Vector2f(m_playercenter.x -5.f, m_playercenter.y - 70.f),
			2.f,50.f,Vector2f(0.f,-1.f),5.f,WEAPON::BOMB));	
		m_shootTimer = 0.f;
	}
	...
	if (m_bombclock.getElapsedTime().asSeconds() >= 1.f)
	{
		for (size_t i = 0; i < m_vecbullet.size();)
		{
			if (m_vecbullet[i].GetType() == WEAPON::BOMB)
			{
				...
				for (int j = 0; j < 12; ++j)
				{
					m_vecbullet.push_back(Bullet(Vector2f(m_vecbullet[i].GetSprite().getPosition()),
...
void Player::AccessoryUpdate()
{
	...
	m_lwing.setPosition(m_playercenter.x + 2 - abs(m_curVelocity.x),
		m_playercenter.y + 45 -abs(m_curVelocity.y));
	m_rwing.setPosition(m_playercenter.x + 2 + abs(m_curVelocity.x),
		m_playercenter.y + 45 - abs(m_curVelocity.y));
...
void Player::AccessoryChange()
{
	...
	{
		if (Keyboard::isKeyPressed(Keyboard::F1)) // LWING
		{
			if (m_lwingcurnum < m_lwingcntmax)
				m_lwingcurnum++;
			else
				m_lwingcurnum = 1;
			m_lwing.setTexture(ResMgr::GetInst()->
				GetTexture("WINGLEFT" + std::to_string(m_lwingcurnum)));
		}
		if (Keyboard::isKeyPressed(Keyboard::F2)) // RWING
		{
			if (m_rwingcurnum < m_rwingcntmax)
				m_rwingcurnum++;
			else
				m_rwingcurnum = 1;
			m_rwing.setTexture(ResMgr::GetInst()->
				GetTexture("WINGRIGHT" + std::to_string(m_rwingcurnum)));

		}
		if (Keyboard::isKeyPressed(Keyboard::F3)) // CPIT
		{
			if (m_cpitcurnum < m_cpitcntmax)
				m_cpitcurnum++;
			else
				m_cpitcurnum = 1;
			m_cPit.setTexture(ResMgr::GetInst()->
				GetTexture("CPIT" + std::to_string(m_cpitcurnum)));

		}
		if (Keyboard::isKeyPressed(Keyboard::F4)) // AURA
		{
			if (m_auracurnum < m_auracntmax)
				m_auracurnum++;
			else
				m_auracurnum = 1;
			m_aura.setTexture(ResMgr::GetInst()->
				GetTexture("AURA" + std::to_string(m_auracurnum)));
		}
		m_changeTimer = 0;
	}
}
...
void Player::TimerUpdate(const float& _dt)
{
	...
	m_timer -= 1.f * _dt;
	m_timer = std::clamp(m_timer, 0.f, m_timermax);
	//if (m_timer > 0.f)
	//{
	//	m_timer -= 1.f;
	//	if (m_timer <= 0.f)
...
void Player::BulletCollisionEnemy(const float& _dt, vector<Enemy>& _vecenemy)
{
	...
	for (size_t i = 0; i < m_vecbullet.size();)
	{
		if (m_vecbullet[i].Update(_dt,m_sprite))
...
bool Player::GainExp(int _exp)
{
	...
	if (m_exp >= m_expNext)
	{
		...
		m_level++;
		m_statPoints++; // todo: 일시정지해서 stat 찍기
		m_str++;
		m_dex++;
		m_endurance++;
		m_defense++;
