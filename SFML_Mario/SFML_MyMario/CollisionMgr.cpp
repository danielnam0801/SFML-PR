...
bool CollisionMgr::IsCollision(Collider* _pLeft, Collider* _pRight)
{
	// 충돌검사 알고리즘
	// AABB 
	Vector2f vLeftPos = _pLeft->GetRect().getPosition();
	Vector2f vRightPos = _pRight->GetRect().getPosition();
	Vector2f vLeftScale = _pLeft->GetScale();
	Vector2f vRightScale = _pRight->GetScale();
	...
	return false;
	/*Vector2f vLeftPos = _pLeft->GetFinalPos();
	Vector2f vRightPos = _pRight->GetFinalPos();
	Vector2f vLeftScale = _pLeft->GetScale();
	Vector2f vRightScale = _pRight->GetScale();
	if (abs(vRightPos.x - vLeftPos.x) <= (vLeftScale.x + vRightScale.x) / 2.f
		&& abs(vRightPos.y - vLeftPos.y) <= (vLeftScale.y + vRightScale.y) / 2.f)
...
bool CollisionMgr::IsCollision(Sprite _pLeft, Sprite _pRight)
{
	if(_pLeft.getGlobalBounds().intersects(_pRight.getGlobalBounds())
		|| _pRight.getGlobalBounds().intersects(_pLeft.getGlobalBounds()))
		return true;
	else
		return false;
}
